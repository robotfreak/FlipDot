...
int printChar8x14(int xOffs, int yOffs, int color, unsigned char c)
{
    unsigned int x, y, w;
...
int printChar10x16(int xOffs, int yOffs, int color, unsigned char c)
{
    unsigned int x, y, wL, wH;
...
int printString(int xOffs, int yOffs, int color, int size, const char *s)
{
    ...
    while ((s[i] != '\0') && (i < 200))
    {
        ...
        case MEDIUM:
            ...
            break;
        case LARGE:
            x = printChar10x16(x, y, color, s[i]);
            break;
        case XLARGE:
